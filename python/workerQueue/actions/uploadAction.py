from workerQueue.actions.baseAction import BaseAction
from api.services import CatalogEntryUploadServices as CatalogEntryUploadServices

UPLOAD_ACTION = "UPLOAD"


class UploadAction(BaseAction):
    def __init__(self):
        super()
        self.LOGGING_AREA = "UploadAction"

    def parse_queue_line(self, queue_line):
        line_vars = queue_line.split(" ")
        action = line_vars[0]
        catalog_entry_upload_id = line_vars[1]
        action_json = {
            "action": action,
            "catalog_entry_upload_id": catalog_entry_upload_id,
        }
        return self.validate_action_json(action_json)

    def compose_queue_line(self, action_json):
        action_json = self.validate_action_json(action_json)
        catalog_entry_upload_id = action_json["catalog_entry_upload_id"]
        queue_line = f"{UPLOAD_ACTION} {catalog_entry_upload_id}"
        return queue_line

    def validate_action_json(self, action_json):
        try:
            if action_json["action"] != UPLOAD_ACTION:
                self.raise_invalid_action_json(action_json)
            catalog_entry_upload_id = action_json["catalog_entry_upload_id"]
            if not self.validate_catalog_entry_upload_id(catalog_entry_upload_id):
                self.raise_invalid_action_json(action_json)
            return {
                "action": UPLOAD_ACTION,
                "catalog_entry_upload_id": catalog_entry_upload_id,
            }
        except:
            self.raise_invalid_action_json(action_json)


uploadAction = UploadAction()
