...
export class SimulationController extends HTMLElement {
  ...
  play() {
    if (this.playing) {
      ...
      if (
        (!this.reverse && nextTimestamp == endDate) ||
        (this.reverse && nextTimestamp == startDate)
      ) {
  ...
  nextFrame() {
    ...
    if (simVars.currentSimulation.includes("lidar")) {
      nextTimestamp = this.simulationSlider.prevTimestamp();
    } else {
      nextTimestamp = this.simulationSlider.nextTimestamp();
    }
  ...
  prevFrame() {
    let prevTimestamp;
    if (simVars.currentSimulation.includes("lidar")) {
      prevTimestamp = this.simulationSlider.nextTimestamp();
    } else {
      prevTimestamp = this.simulationSlider.prevTimestamp();
    }
