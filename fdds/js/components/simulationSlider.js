...
export class SimulationSlider extends Slider {
    ...
    controllers.loadingProgress.subscribe(() => {
      ...
      if (progress > 0) {
        if (progress >= this.progressCheck) {
          this.progressCheck =
            Math.floor((this.progressCheck + 0.01) * 100) / 100;
  ...
  initializeStartSetter() {
    ...
    this.setBoundingDatePointerDown(
      controllers.startDate,
      sliderStart,
      updateCallback,
      finishedCallback,
  ...
  getLeftOfDate(date) {
    ...
    let left = Math.floor(
      (index / (simVars.sortedTimestamps.length - 1)) * this.sliderWidth * 0.95,
    );
  ...
  setLoadProgress(progress) {
    ...
    if (simVars.currentSimulation.includes("lidar")) {
      let endDate = controllers.endDate.getValue();
      let endIndex = simVars.sortedTimestamps.indexOf(endDate) + 1;
      let left =
        Math.floor(
          (endIndex / (simVars.sortedTimestamps.length - 1)) *
          this.sliderWidth *
          0.95,
        ) + 14;

      let right = Math.floor(this.sliderWidth - left) - 2;
      progressBar.style.removeProperty("left");
      progressBar.style.right = right + "px";
    } else {
      let startDate = controllers.startDate.getValue();
      let startIndex = simVars.sortedTimestamps.indexOf(startDate);
      let left = Math.floor(
        (startIndex / (simVars.sortedTimestamps.length - 1)) *
        this.sliderWidth *
        0.95,
      );
