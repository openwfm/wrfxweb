...
export class SimulationLayer {
  ...
  removeLayer() {
    ...
    simVars.overlayOrder.splice(
      simVars.overlayOrder.indexOf(this.layerName),
      1,
    );
  }
  ...
  async rgbValueAtLocation(timestamp, coords) {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        this.imgCanvas
          .getContext("2d")
          .drawImage(img, imgX, imgY, 1, 1, 0, 0, 1, 1);
        let pixelData = this.imgCanvas
          .getContext("2d")
  ...
  mapRGBsToColorbarValues(colorbarImg, timestamp) {
    ...
    let [top, bottom] = this.getColorbarVerticalBoundsAndMapRGBsToHeights(
      horizontalCenterOfColorbar,
      clrbarMap,
    );
  ...
  getColorbarHorizontalBounds() {
    ...
    for (let x = this.clrbarCanvas.width - 1; x > 0; x--) {
      let colorbarData = this.clrbarCanvas
        .getContext("2d")
  ...
  getColorbarVerticalBoundsAndMapRGBsToHeights(
    horizontalCenterOfColorbar,
    clrbarMap,
  ) {
    let top = 0;
    ...
    for (let j = 0; j < this.clrbarCanvas.height; j++) {
      let [r, g, b, alpha] = this.clrbarCanvas
        .getContext("2d")
        .getImageData(horizontalCenterOfColorbar, j, 1, 1).data;
      if (top == 0) {
        if (r + g + b != 0) {
          top = j + 1;
        }
  ...
  convertHeightsToProportionalHeights(clrbarMap, top, bottom) {
    const computeLocation = (key) =>
      1 - (clrbarMap[key] - top) / (bottom - top);
  ...
  convertProportionalHeightsToColorbarValues(clrbarMap, timestamp) {
    ...
    let levelMap = {};
    let levelsAreStratified = this.areColorbarLevelsStratified(
      clrbarMap,
      levels,
      levelMap,
    );
    let [firstLevel, lastLevel] = this.mapColorbarTickLocationsToValues(
      clrbarMap,
      levels,
      levelMap,
    );
  ...
  getColorbarLevelsAtTimestamp(timestamp) {
    let rastersAtTimestamp = simVars.rasters[this.domain][timestamp];
  ...
  mapColorbarTickLocationsToValues(clrbarMap, levels, levelMap) {
    ...
    let location;
    let lastLevel = [];
    const computeLocation = (y) =>
      1 - (y - clrbarMap.start) / (clrbarMap.end - clrbarMap.start);
    for (let y = 0; y < this.clrbarCanvas.height; y++) {
      ...
      if (colorbarData[3] != 0) {
        ...
        while (colorbarData[3] != 0 && y < this.clrbarCanvas.height) {
          y += 1;
          markHeight += 1;
          colorbarData = this.clrbarCanvas
            .getContext("2d")
            .getImageData(x, y, 1, 1).data;
        }
        if (levelIndex == 0) {
  ...
  drawColorbarCanvas(colorbarImg) {
    this.clrbarCanvas
      .getContext("2d")
      .clearRect(0, 0, this.clrbarCanvas.width, this.clrbarCanvas.height);
    if (colorbarImg == null) {
      return;
    }
    ...
    this.clrbarCanvas
      ...
      .drawImage(
        ...
        0,
        0,
        this.clrbarCanvas.width,
        this.clrbarCanvas.height,
      );
  }
}
