...
export class LayerController extends HTMLElement {
  ...
  initializeLayerButton() {
    ...
    layersButton.onpointerdown = (e) => {
      const layersSelector = document.querySelector(
  ...
  subscribeToTopLayerOpacity() {
    controllers.opacity.subscribe(() => {
      ...
      if (simVars.overlayOrder.length > 0) {
        ...
        let topLayerName =
          simVars.overlayOrder[simVars.overlayOrder.length - 1];
  ...
  createOpacitySlider() {
    const opacitySliderContainer = this.querySelector(
  ...
  resetLayerController() {
    ...
    this.querySelector("#layer-controller-container").classList.remove(
      "hidden",
    );
  ...
  switchDomain() {
    ...
    let previouslyAddedLayerNames = simVars.overlayOrder.filter((overlay) => {
      return (
        overlay in this.overlayDict[currentDomain] ||
        overlay in this.rasterDict[currentDomain]
      );
    });
  ...
  bringMostRecentColorbarLayerToFront() {
    ...
    for (let i = simVars.overlayOrder.length - 1; i >= 0; i--) {
      ...
      if ("colorbar" in rasters_now[simVars.overlayOrder[i]]) {
        ...
        colorbarSrc =
          simVars.rasterBase + rasters_now[simVars.overlayOrder[i]].colorbar;
        break;
      }
    }
  ...
  updateToCurrentTimestamp() {
    ...
    for (let addedLayerName of simVars.overlayOrder) {
      ...
      if (
        !shouldLoadAtEnd &&
        !addedLayer.timestampIsPreloaded(currentTimestamp)
      ) {
    ...
    if (shouldLoadAtEnd) {
      let endTime =
        simVars.sortedTimestamps[simVars.sortedTimestamps.length - 1];
